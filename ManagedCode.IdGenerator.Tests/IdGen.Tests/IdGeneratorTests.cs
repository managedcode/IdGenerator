...
public class IdGeneratorTests
{
    ...
    public void Sequence_ShouldReset_EveryNewTick()
    {
        ...
        Assert.Equal((1 << (g.Options.IdStructure.GeneratorIdBits + g.Options.IdStructure.SequenceBits)) + 0, g.CreateId());
        Assert.Equal((1 << (g.Options.IdStructure.GeneratorIdBits + g.Options.IdStructure.SequenceBits)) + 1, g.CreateId());
    }
    ...
    public void GeneratorId_ShouldBePresent_InID1()
    {
        ...
        Assert.Equal(((1 << g.Options.IdStructure.GeneratorIdBits) - 1) << g.Options.IdStructure.SequenceBits, g.CreateId());
    }
    ...
    public void GeneratorId_ShouldBeMasked_WhenReadFromProperty()
    {
        ...
        // Make sure all expected bits are set
        Assert.Equal((1 << g.Options.IdStructure.GeneratorIdBits) - 1, g.Id);
    }
    ...
    public void Constructor_UsesCorrectId()
        => Assert.Equal(42, new IdGenerator(42).Id);

    [Fact]
    public void CreateId_Throws_OnSequenceOverflow() => Assert.Throws<SequenceOverflowException>(() =>
    {
        var ts = new MockTimeSource();
    ...
    public void TryCreateId_Returns_False_OnSequenceOverflow()
    {
        ...
        for (var i = 0; i < 4; i++)
        {
            Assert.True(g.TryCreateId(out var _));
        }

        // However, if we invoke once more we should get an SequenceOverflowException
        // which should be indicated by the false return value
        Assert.False(g.TryCreateId(out var _));
    }
    ...
    [Fact]
    public void CreateId_Throws_OnClockBackwards() => Assert.Throws<InvalidSystemClockException>(() =>
    {
    ...
    [Fact]
    public void CreateId_Throws_OnTimestampWraparound() => Assert.Throws<InvalidSystemClockException>(() =>
    {
    ...
    public void TryCreateId_Returns_False_OnTimestampWraparound()
    {
        ...
        Assert.True(g.TryCreateId(out var _));    // Should succeed;
        ts.NextTick();
        Assert.False(g.TryCreateId(out var _));   // Should fail
    }
    ...
    public void FromId_Returns_CorrectValue()
    {
        ...
        Assert.Equal(34, target.SequenceNumber);                                     // We generated 35 id's in the same tick, so sequence should be at 34.
        Assert.Equal(234, target.GeneratorId);                                       // With generator id 234
        Assert.Equal(epoch.Add(TimeSpan.FromSeconds(5 * 7)), target.DateTimeOffset); // And the clock was at 5 ticks, with each tick being
    ...
    public void CreateId_Waits_OnSequenceOverflow()
    {
        ...
        Assert.Equal(1, g.CreateId());
        Assert.Equal(2, g.CreateId());
        Assert.Equal(3, g.CreateId());
        Assert.Equal(4, g.CreateId());   // This should trigger a spinwait and return the next ID
        Assert.Equal(5, g.CreateId());
    }
}