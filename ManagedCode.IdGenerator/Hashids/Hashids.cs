...
/// <summary>
///     Generates YouTube-like hashes from one or many numbers. Use hashids when you do not want to expose your database
///     ids to the user.
/// </summary>
public class Hashids : IHashids
{
        ...
        string seps = DEFAULT_SEPS)
    {
        ...
        {
            throw new ArgumentException($"Alphabet must contain at least {MIN_ALPHABET_LENGTH:N0} unique characters.", nameof(alphabet));
        }
        ...
        {
            throw new ArgumentException($"Alphabet must contain at least {MIN_ALPHABET_LENGTH:N0} unique characters that are also not present in .",
                nameof(alphabet));
        }

        ConsistentShuffle(_seps, _salt);
        ...
        ConsistentShuffle(_alphabet, _salt);
    ...
    {
        return EncodeLong(Array.ConvertAll(numbers, n => (long)n));
    }
    ...
    {
        return Array.ConvertAll(GetNumbersFrom(hash), n => (int)n);
    }
    ...
    public virtual string EncodeHex(string hex)
    {
        ...
        for (var i = 0; i < numbers.Length; i++)
        {
            ...
            var number = Convert.ToInt64(concat, 16);
            numbers[i] = number;
        }
    ...
    private int BuildReversedHash(long input, ReadOnlySpan<char> alphabet, Span<char> hashBuffer)
    {
        ...
        do
        {
            var idx = (int)(input % _alphabet.Length);
    ...
    private long GetNumberFrom(string hash)
    {
        ...
        var buffer = _alphabet.Length < MAX_STACKALLOC_SIZE ? stackalloc char[_alphabet.Length] : new char[_alphabet.Length];
        buffer[0] = lottery;
    ...
    private long[] GetNumbersFrom(string hash)
    {
        ...
        var hashBuffer = hash.Length < MAX_STACKALLOC_SIZE ? stackalloc char[hash.Length] : new char[hash.Length];
