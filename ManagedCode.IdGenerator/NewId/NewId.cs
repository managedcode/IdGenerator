...
public readonly struct NewId :
    ...
    IFormattable
{
    public static readonly NewId Empty = new(0, 0, 0, 0);

    private static readonly INewIdFormatter BraceFormatter = new DashedHexFormatter('{', '}');
    private static readonly INewIdFormatter DashedHexFormatter = new DashedHexFormatter();
    private static readonly INewIdFormatter HexFormatter = new HexFormatter();
    private static readonly INewIdFormatter ParenFormatter = new DashedHexFormatter('(', ')');
    ...
    private static IWorkerIdProvider WorkerIdProvider => _workerIdProvider ??= new BestPossibleWorkerIdProvider();

    private static IProcessIdProvider ProcessIdProvider { get; set; }

    private static ITickProvider TickProvider => _tickProvider ??= new DateTimeTickProvider();
    ...
    private static readonly ThreadLocal<byte[]> _formatterArray = new(() => new byte[16]);
    ...
    private static void FromByteArray(in byte[] bytes, out int a, out int b, out int c, out int d)
    {
